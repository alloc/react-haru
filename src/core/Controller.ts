...
import { OneOrMore, UnknownProps, Lookup, Falsy } from '../types'
import {
  is,
  each,
  noop,
...
export async function flushUpdate(
  ...
  const defaults = is.plainObject(props.default) && props.default
  ...
  const asyncTo = is.array(to) || is.function(to) ? to : undefined
  if (asyncTo) {
    props.to = undefined
...
export function getSprings<State extends Lookup>(
  ...
  if (props) {
    each(toArray(props), (props: any) => {
      if (is.undefined(props.keys)) {
        props = createUpdate(props)
      }
      if (!is.plainObject(props.to)) {
