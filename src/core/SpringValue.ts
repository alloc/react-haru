...
export class SpringValue<T = any> extends FrameValue<T> {
  ...
  constructor(arg1?: any, arg2?: any) {
    super()
    if (is.defined(arg1) || is.defined(arg2)) {
      const props = is.plainObject(arg1) ? { ...arg1 } : { ...arg2, from: arg1 }
      if (is.undefined(props.default)) {
        props.default = true
      }
      this.start(props)
    }
    ...
    anim.values.forEach((node, i) => {
      ...
      if (!finished) {
        ...
        const v0 =
          node.v0 != null
            ? node.v0
            : (node.v0 = is.array(config.velocity)
                ? config.velocity[i]
                : config.velocity)
        ...
        // Spring easing
          ...
          const canBounce = is.defined(bounceFactor)
  ...
  start(to?: T | SpringUpdate<T>, arg2?: SpringProps<T>) {
    ...
    if (is.defined(to)) {
      queue = [is.plainObject(to) ? to : { ...arg2, to }]
    } else {
      queue = this.queue || []
      this.queue = []
    }
  ...
  protected _prepareNode(props: {
    ...
    to = is.plainObject(to) ? to[key] : to
    if (to == null || isAsyncTo(to)) {
      to = undefined
    }

    from = is.plainObject(from) ? from[key] : from
    if (from == null) {
      from = undefined
    }
  ...
  protected _merge(
    ...
    const hasToProp = is.defined(range.to)

    /** The "from" prop is defined. */
    const hasFromProp = is.defined(range.from)
    ...
    if (hasFromProp && !hasToProp && (!props.default || is.undefined(to))) {
      to = from
    }
    ...
    const reset =
      ...
      is.undefined(props.reset)
        ? hasFromProp && !props.default
        : is.defined(from) && matchProp(props.reset, key)
    ...
    const isAnimatable =
      is.number(goal) || is.array(goal) || isAnimatedString(goal)
...
export function createUpdate(props: any) {
  ...
  if (is.plainObject(to)) findDefined(to, keys)
  if (is.plainObject(from)) findDefined(from, keys)
...
export function declareUpdate(props: any) {
  ...
  if (is.undefined(update.default)) {
