...
import { AnyFn, OneOrMore, Lookup } from '../types'
...
export function callProp<T>(
  ...
  return is.function(value) ? value(...args) : value
}
...
export const matchProp = (
  ...
  !!(
    key &&
    value &&
    (is.function(value) ? value(key) : toArray(value).includes(key))
  )

export const resolveProp = <T>(
  prop: T | Lookup<T> | undefined,
  key: string | undefined
) => (is.plainObject(prop) ? key && (prop as any)[key] : prop)
...
export const hasDefaultProp = <T extends Lookup>(props: T, key: keyof T) =>
  is.defined(getDefaultProp(props, key))
...
export function isAsyncTo(to: any) {
  return is.function(to) || (is.array(to) && is.plainObject(to[0]))
}
