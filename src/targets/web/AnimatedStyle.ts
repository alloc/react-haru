...
import type { Lookup, OneOrMore } from '../../types'
import { each, toArray, eachProp } from '../../shared/helpers'
...
const addUnit = (value: Value, unit: string): string | 0 =>
  is.number(value) && value !== 0 ? value + unit : value
  ...
  protected _get() {
    ...
    each(this.inputs, (input, i) => {
      ...
      const [t, id] = this.transforms[i](
        is.array(arg1) ? arg1 : input.map(getFluidValue)
      )
      transform += ' ' + t
